  ...
  inviteMailer.sendMail(email, user, pronite, qr, () => {
    ...
    dynamoDB.update(inviteParams, function (err, data) {
      if (err) {
        ...
        return res.json({
          error: false,
          message: 'Invitation sent successfully! If your invitee doesn\'t receive a mail, you can send them the following link: http://rdv-iitd.com/api/invitee-pdf?qr=' + qr
        });
      }
    });
  });
